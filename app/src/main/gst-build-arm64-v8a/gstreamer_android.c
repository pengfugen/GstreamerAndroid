...
static void
LOGD_logcat (GstDebugCategory * category, GstDebugLevel level,
    ...
    GObject * object, GstDebugMessage * message, gpointer unused)
{
  ...
  if (level > LOGD_category_get_threshold (category))
    return;
  ...
  tag = g_strdup_printf ("GStreamer+%s",
      LOGD_category_get_name (category));

  if (object) {
    gchar *obj;

    if (GST_IS_PAD (object) && GST_OBJECT_NAME (object)) {
      obj = g_strdup_printf ("<%s:%s>", LOGD_PAD_NAME (object));
    ...
    __android_log_print (android_log_level, tag,
        ...
        file, line, function, obj, LOGD_message_get (message));
    ...
    __android_log_print (android_log_level, tag,
        ...
        GST_TIME_ARGS (elapsed), g_thread_self (),
        file, line, function, LOGD_message_get (message));
  }
  g_free (tag);
}
...
gst_android_init (JNIEnv * env, jobject context)
{
  ...
  LOGD_set_active (TRUE);
  LOGD_set_default_threshold (GST_LEVEL_WARNING);
  LOGD_remove_log_function (LOGD_log_default);
  LOGD_add_log_function ((GstLogFunction) LOGD_logcat, NULL, NULL);
