...
static JNIEnv *attach_current_thread (void) {
  ...
  LOGI("Attaching thread %p", g_thread_self ());
...
static void detach_current_thread (void *env) {
  LOGI("Detaching thread %p", g_thread_self ());
...
static void set_ui_message (const gchar *message, CustomData *data) {
  ...
  LOGI("Setting message to: %s", message);
...
static gboolean refresh_ui (CustomData *data) {
  ...
  if (!GST_CLOCK_TIME_IS_VALID (data->duration)) {
    if (!gst_element_query_duration (data->pipeline, GST_FORMAT_TIME, &data->duration)) {
      LOGE("Could not query current duration");
    }
  }
...
static void execute_seek (gint64 desired_position, CustomData *data) {
  ...
  if (GST_CLOCK_TIME_IS_VALID (data->last_seek_time) && diff < SEEK_MIN_DELAY) {
    ...
    LOGI("Throttling seek to %" GST_TIME_FORMAT ", will be in %" GST_TIME_FORMAT,
        GST_TIME_ARGS (desired_position), GST_TIME_ARGS (SEEK_MIN_DELAY - diff));
  } else {
    /* Perform the seek now */
    LOGI("Seeking to %" GST_TIME_FORMAT, GST_TIME_ARGS (desired_position));
...
static gboolean delayed_seek_cb (CustomData *data) {
  LOGI("Doing delayed seek to %" GST_TIME_FORMAT, GST_TIME_ARGS (data->desired_position));
...
static void check_media_size (CustomData *data) {
  ...
  if (gst_video_info_from_caps (&info, caps)) {
    ...
    LOGI("Media size is %dx%d, notifying application", info.width, info.height);
...
static void check_initialization_complete (CustomData *data) {
  ...
  if (!data->initialized && data->native_window && data->main_loop) {
    LOGI("Initialization complete, notifying application. native_window:%p main_loop:%p", data->native_window, data->main_loop);
...
static void *app_function (void *userdata) {
  ...
  LOGI("Creating pipeline in CustomData at %p", data);
  ...
  LOGI("Entering main loop... (CustomData:%p)", data);
...
static void gst_native_init (JNIEnv* env, jobject thiz) {
  ...
  LOGI("Created CustomData at %p", data);
  data->app = (*env)->NewGlobalRef (env, thiz);
  LOGI("Created GlobalRef for app object at %p", data->app);
...
static void gst_native_finalize (JNIEnv* env, jobject thiz) {
  ...
  LOGI("Deleting GlobalRef for app object at %p", data->app);
  (*env)->DeleteGlobalRef (env, data->app);
  LOGI("Freeing CustomData at %p", data);
  g_free (data);
  SET_CUSTOM_DATA (env, thiz, custom_data_field_id, NULL);
  LOGI("Done finalizing");
}
...
void gst_native_set_uri (JNIEnv* env, jobject thiz, jstring uri) {
  ...
  LOGI("Setting URI to %s", char_uri);
...
void gst_native_set_position (JNIEnv* env, jobject thiz, int milliseconds) {
  ...
  if (data->state >= GST_STATE_PAUSED) {
    ...
    LOGI("Scheduling seek to %" GST_TIME_FORMAT " for later", GST_TIME_ARGS (desired_position));
    data->desired_position = desired_position;
  }
}
...
static void gst_native_surface_init (JNIEnv *env, jobject thiz, jobject surface) {
  ...
  if (data->native_window) {
    ...
    if (data->native_window == new_native_window) {
      LOGI("New native window is the same as the previous one %p", data->native_window);
      if (data->pipeline) {
      ...
      LOGI("Released previous native window %p", data->native_window);
...
static void gst_native_surface_finalize (JNIEnv *env, jobject thiz) {
  CustomData *data = GET_CUSTOM_DATA (env, thiz, custom_data_field_id);
  if (!data) return;
  LOGI("Releasing Native Window %p", data->native_window);
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass klass = (*env)->FindClass (env, "com/pfg/gstreamerandroid/Tutorial4");
  (*env)->RegisterNatives (env, klass, native_methods, G_N_ELEMENTS(native_methods));

  setenv("GST_DEBUG", "*:6", 5 );
  setenv("GST_DEBUG_NO_COLOR", "1", 5);
