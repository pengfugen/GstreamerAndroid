...
LOGD_CATEGORY_STATIC (debug_category);
...
static void execute_seek (gint64 desired_position, CustomData *data) {
  ...
  if (GST_CLOCK_TIME_IS_VALID (data->last_seek_time) && diff < SEEK_MIN_DELAY) {
    ...
    LOGD ("Throttling seek to %" GST_TIME_FORMAT ", will be in %" GST_TIME_FORMAT,
        GST_TIME_ARGS (desired_position), GST_TIME_ARGS (SEEK_MIN_DELAY - diff));
  } else {
    /* Perform the seek now */
    LOGD ("Seeking to %" GST_TIME_FORMAT, GST_TIME_ARGS (desired_position));
...
static gboolean delayed_seek_cb (CustomData *data) {
  LOGD ("Doing delayed seek to %" GST_TIME_FORMAT, GST_TIME_ARGS (data->desired_position));
...
static void check_media_size (CustomData *data) {
  ...
  if (gst_video_info_from_caps(&info, caps)) {
    ...
    LOGD ("Media size is %dx%d, notifying application", info.width, info.height);
...
static void check_initialization_complete (CustomData *data) {
  ...
  if (!data->initialized && data->native_window && data->main_loop) {
    LOGD ("Initialization complete, notifying application. native_window:%p main_loop:%p", data->native_window, data->main_loop);
...
static void *app_function (void *userdata) {
  ...
  LOGD ("Creating pipeline in CustomData at %p", data);
  ...
  LOGD ("Entering main loop... (CustomData:%p)", data);
...
static void gst_native_init (JNIEnv* env, jobject thiz) {
  ...
  LOGD_CATEGORY_INIT (debug_category, "tutorial-5", 0, "Android tutorial 5");
  LOGD_set_threshold_for_name("tutorial-5", GST_LEVEL_DEBUG);
...
void gst_native_set_uri (JNIEnv* env, jobject thiz, jstring uri) {
  ...
  LOGD ("Setting URI to %s", char_uri);
...
void gst_native_set_position (JNIEnv* env, jobject thiz, int milliseconds) {
  ...
  if (data->state >= GST_STATE_PAUSED) {
    ...
    LOGD ("Scheduling seek to %" GST_TIME_FORMAT " for later", GST_TIME_ARGS (desired_position));
    data->desired_position = desired_position;
  }
}
...
static void gst_native_surface_init (JNIEnv *env, jobject thiz, jobject surface) {
  ...
  if (data->native_window) {
    ...
    if (data->native_window == new_native_window) {
      ...
      LOGD ("Released previous native window %p", data->native_window);
...
jint JNI_OnLoad(JavaVM *vm, void *reserved) {
  ...
  jclass klass = (*env)->FindClass (env, "com/pfg/gstreamerandroid/Tutorial5");
  (*env)->RegisterNatives (env, klass, native_methods, G_N_ELEMENTS(native_methods));

  pthread_key_create (&current_jni_env, detach_current_thread);

  setenv("LOGD", "*:6", 3);
  setenv("LOGD_NO_COLOR", "1", 3);
